...
macOS BSD grep does not support some extended or PCRE patterns and may fail with repetition operator errors. Use scripts/detect_lwa_secrets.py to discover secret names in a portable way.
...

{
  ...
  read -r LWA_REFRESH_EU_SECRET
  read -r LWA_REFRESH_NA_SECRET
  read -r LWA_REFRESH_GENERIC_SECRET
} < <(python3 scripts/detect_lwa_secrets.py)

if [ "$SCOPE" = "EU" ] && [ -n "$LWA_REFRESH_EU_SECRET" ]; then
  REFRESH_SECRET=$LWA_REFRESH_EU_SECRET
  REFRESH_ENV=LWA_REFRESH_TOKEN_EU
elif [ "$SCOPE" = "NA" ] && [ -n "$LWA_REFRESH_NA_SECRET" ]; then
  REFRESH_SECRET=$LWA_REFRESH_NA_SECRET
  REFRESH_ENV=LWA_REFRESH_TOKEN_NA
elif [ -n "$LWA_REFRESH_GENERIC_SECRET" ]; then
  REFRESH_SECRET=$LWA_REFRESH_GENERIC_SECRET
  REFRESH_ENV=LWA_REFRESH_TOKEN
else
  echo "No refresh token secret detected"
  exit 1
fi
...
gcloud run services update "$SERVICE_NAME" \
  --project "$PROJECT_ID" \
  --region "$REGION" \
  --update-secrets "LWA_CLIENT_ID=$LWA_CLIENT_ID_SECRET:latest,LWA_CLIENT_SECRET=$LWA_CLIENT_SECRET_SECRET:latest,$REFRESH_ENV=$REFRESH_SECRET:latest,AWS_ACCESS_KEY_ID=<PUT_YOUR_AWS_ACCESS_KEY_ID_SECRET>:latest,AWS_SECRET_ACCESS_KEY=<PUT_YOUR_AWS_SECRET_ACCESS_KEY_SECRET>:latest" \
  --set-env-vars "AWS_REGION=<PUT_YOUR_AWS_REGION>,AWS_SESSION_TOKEN=<PUT_YOUR_AWS_SESSION_TOKEN>,BQ_PROJECT=$PROJECT_ID"
